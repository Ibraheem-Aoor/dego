                    ...
                    <div class="d-grid gap-3 gap-lg-5">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <input type="hidden" name="home_name" value="{{ $page->home_name }}">
                                <div class="row mb-4">
                    ...
                    <div class="tom-select-custom">
                        <select class="js-select form-select" id="customBlockSelect" data-hs-tom-select-options='{
                            ...
                            @foreach($sections as $sectionName => $sectionData)
                                <option value="[[{{ $sectionName }}]]" data-section="{{ json_encode($sectionData) }}">{{ ucwords($sectionData['basename']) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="modal-footer d-flex justify-content-between">
                    <button id="contentImage" type="button" class="btn btn-success contentImageView">
                        @lang("View Content")
                    </button>
                    <div>
                        <button type="button" class="btn btn-white" data-bs-dismiss="modal">@lang("Close")</button>
                        <button id="insertCustomBlockButton" type="button" class="btn btn-primary">@lang("Insert Section")</button>
                    </div>
                </div>
            </div>
        </div>
...
@push('js-lib')
    ...
    <script src="{{ asset('assets/global/js/jquery.magnific-popup.js') }}" defer></script>
@endpush
...
@push('script')
    ...
    <script>
        ...
        $(document).ready(function () {
            HSCore.components.HSTomSelect.init('#customBlockSelect', {
                maxOptions: 250,
        ...
        function initializeContentImageGallery() {
            const contentImageButton = document.getElementById('contentImage');
            const customBlockSelect = document.getElementById('customBlockSelect');

            customBlockSelect.addEventListener('change', function() {
                let selectedOption = this.options[this.selectedIndex];
                let sectionData = selectedOption.getAttribute('data-section');
                contentImageButton.setAttribute('data-image', sectionData);
            });

            let initialOption = customBlockSelect.options[0];
            let initialSectionData = initialOption.getAttribute('data-section');
            contentImageButton.setAttribute('data-image', initialSectionData);

            $('.contentImageView').on('click', function() {
                let sectionData = JSON.parse($(this).attr('data-image'));
                let imageArray = sectionData['image'];

                let items = Object.entries(imageArray).map(function([title, imageUrl]) {
                    return {
            ...
            if (!contentImageButton.getAttribute('data-image')) {
                contentImageButton.click();
            }
        }
        document.addEventListener('DOMContentLoaded', initializeContentImageGallery);
    </script>
