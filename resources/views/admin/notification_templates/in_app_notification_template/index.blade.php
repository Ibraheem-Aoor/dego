...
@section('page_title', __('In App Notification Templates'))
@section('content')
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active"
                                aria-current="page">@lang("In App Notification Templates")</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">@lang("In App Notification Templates")</h1>
                </div>
            </div>
        </div>
                                ...
                                <div class=" table-responsive datatable-custom  ">
                                    <table id="datatable"
                                        ...
                                        @forelse($templates as $template)
                                            <tr>
                                                <td data-label="Name">{{ __($loop->index + 1) }} </td>
                                                <td data-label="Name">{{ __($template->name) }} </td>
                                                <td data-label="Name">
                                                        <span class="badge bg-soft-{{ $template->status['in_app'] == 1 ? "success" :  "danger"  }} text-{{ $template->status['in_app'] == 1 ? "success" :  "danger" }}">
                                                        <span class="legend-indicator bg-{{ $template->status['in_app'] == 1 ? "success" :  "danger" }}"></span> {{ __($template->status['in_app'] == 1 ? "Active" :  "Inactive") }}
                                                    </span>
                                                </td>
                                                <td data-label="Action">
                                                    <a class="btn btn-white btn-sm"
                                                       href="{{ route('admin.in.app.notification.template.edit', $template->id) }}">
