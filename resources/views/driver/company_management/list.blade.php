                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item"><a class="breadcrumb-link"
                                                           href="javascript:void(0);">@lang('Company Management')</a></li>
                            <li class="breadcrumb-item active" aria-current="page">@lang('Agents')</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">@lang('Companies List')</h1>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-6 col-lg-3 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">@lang('Total Companies')</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{ fractionNumber($company_recored[0]['totalUser'], false) }}</span>
                                <span
                                    class="text-body fs-5 ms-1">@lang("from") {{ fractionNumber($company_recored[0]['totalUserWithTrashed'], false) }}</span>
                            </div>
                            <div class="col-auto">
                              <span class="badge bg-soft-success text-success p-1">
                                <i class="bi-graph-up"></i> {{ fractionNumber($company_recored[0]['totalUserPercentage']) }}%
                              </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-sm-6 col-lg-3 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">@lang('Active Companies')</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{fractionNumber( $company_recored[0]['activeUser'], false) }}</span>
                                <span class="text-body fs-5 ms-1">@lang("from") {{ $company_recored[0]['totalUser'] }}</span>
                            </div>
                            <div class="col-auto">
                              <span class="badge bg-soft-success text-success p-1">
                                <i class="bi-graph-up"></i> {{ number_format($company_recored[0]['activeUserPercentage'], 2) }}%
                              </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{ fractionNumber($company_recored[0]['todayJoin'],false) }}</span>
                                <span class="text-body fs-5 ms-1">from {{ $company_recored[0]['todayJoin'] }}</span>
                            </div>
                            <div class="col-auto">
                          <span class="badge bg-soft-info text-info p-1">
                            <i class="bi-graph-down"></i> {{ number_format($company_recored[0]['todayJoinPercentage'], 2) }}%
                          </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span
                                    class="js-counter display-4 text-dark">{{ fractionNumber($company_recored[0]['deactivateUser'], false) }}</span>
                                <span class="text-body fs-5 ms-1">from {{ $company_recored[0]['totalUser'] }}</span>
                            </div>
                            <div class="col-auto">
                                <span class="badge bg-soft-danger text-danger p-1">{{ number_format($company_recored[0]['deactivateUserPercentage'], 2) }}%</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                ...
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <a class="btn btn-primary btn-sm" href="{{ route('agent.company.add') }}">
                        <i class="bi-person-plus-fill me-1"></i> @lang('Add Company')
                    </a>
                </div>
            </div>
    ...
    <script>
        $(document).on('ready', function () {
            ...
            HSCore.components.HSDatatables.init($('#datatable'), {
                ...
                ordering: false,
                ajax: {
                    url: "{{ route("agent.company.search") }}",
            ...
            $(document).on('click', '#filter_button', function () {
                ...
                datatable.ajax.url("{{ route('agent.company.search') }}" + "?filterStatus=" + filterSelectedStatus +
            ...
            $(document).on('click', '.delete-multiple', function (e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('agent.company.delete.multiple') }}",
                    data: {strIds: strIds},
                    datatType: 'json',
                    type: "post",
