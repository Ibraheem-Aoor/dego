...
@section('content')
    ...
    @include(template() . 'checkout.car.form')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            flatpickr(dateInput, {
                ...
                onClose: function(selectedDates, dateStr) {
                    $.ajax({
                        url: '{{ route('user.car.date.update') }}',
                        type: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrfToken
                        },
                        data: {
                            id: "{{ encrypt($instant->id) }}",
                            date: dateStr,
                            is_update_in_checkout: true,
                        },
                        success: function(response) {
                            if (response.status) {
                                document.getElementById('editUserInfo').href = response.edit_userinfo_url;
        ...
        $(document).ready(function() {
            ...
            function handleDiscountErrors(message, color) {
                $('.discountMessage').text(message).css({
                    'color': color
                });
            }
                    ...
                    success: function(response) {
                        if (response) {
                            ...
                            handleDiscountErrors(
                                `You got ${currencySymbol}${discount} discount`, 'green');
            ...
            $(document).on('change, input',
                "#amount, #supported_currency, .select-payment-method, #supported_crypto_currency",
                function(e) {
