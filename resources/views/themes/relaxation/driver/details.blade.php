...
@section('content')
        ...
        <div class="container">
            <div class="destination-details-title-box">
                <div class="destination-details-title">
                    <h3>@lang($driver->car->name)</h3>
                </div>
                {{-- <div class="destination-details-review">
                    @if ($driver->review_average > 0)
                        <ul class="review-star">
                            {!! displayStarRating($driver->review_average) !!}
                            <p class="mb-0">{{$driver->review_count .' reviews' }} </p>
                        </ul>
                    @endif

                    <div class="destination-details-review-text">
                        <span><i class="fa-regular fa-location-dot"></i>
                            {{ optional($driver->cityTake)->name .', '.optional($driver->stateTake)->name .', '. optional($driver->countryTake)->name }}
                        </span>
                    </div>
                </div> --}}
            </div>
    ...
    @if ($driver?->related_packages?->count() > 0)
