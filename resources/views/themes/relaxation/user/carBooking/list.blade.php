...
@section('content')
            ...
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a class="text-capitalize"
                            href="{{ route('page', '/') }}">@lang('home')</a>
                    </li>
                    <li class="breadcrumb-item active">@lang('Car Booking History')</li>
                </ol>
            </nav>
        </div>
                                ...
                                @forelse($bookings as $item)
                                    <tr>
                                        ...
                                        <td data-label="@lang('Destination')">
                                            <a href="{{ route('car.details', encrypt(optional($item->car)->id)) }}">
                                                <div class="d-flex justify-content-start align-items-center gap-2">
                                                    <img class="favouriteImage"
                                                        src="{{ getFile(optional($item->car)->thumb_driver, optional($item->car)->thumb) }}" />
                                                    <p class="mb-0">@lang($item->car->name)</p>
                                                </div>
                                            </a>
                                        </td>
                                        <td data-label="@lang('Paid Amount')">
                                            <span>{{ currencyPosition($item->total_price) }}</span>
                                        </td>
                                        <td data-label="@lang('Total Person')">
                                            <span>{{ $item->getDuration() . ' Days' }}</span>
                                        </td>
                                        <td data-label="@lang('Tour Date')"><span>{{ $item->getDatesString() }}</span></td>
                                        <td data-label="@lang('Status')">
                                            ...
                                            @elseif($item->maxBookingDate() < now() && $item->status != 4 && $item->status != 2 && $item->status != 1)
                                        ...
                                        <td class="text-center" data-label="@lang('Action')">
                                            <a class="btn btn-outline-light btn-sm text-dark bookingView pointer"
                                                data-bs-toggle="tooltip" data-bs-original-title="@lang('Booking Details')"
                                                data-package="{{ route('car.details', encrypt(optional($item->car)->id) ?? '') }}"
                                                data-bs-toggle="modal" data-bs-target="#viewInformation"
                                                data-title="{{ $item->car->name }}" data-date="{{ $item->getDatesLabel() }}"
                                                data-duration="{{ $item->getDuration(true) }}"
                                                data-start_price="{{ currencyPosition($item->start_price) }}"
                                                data-total_price="{{ currencyPosition($item->total_price) }}"
                                                data-trx_id="{{ $item->trx_id }}" data-status="{{ $item->status }}"><i
                                                    class="fa-regular fa-eye pe-1"></i>@lang('Details')</a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
    ...
    @include(template() . 'user.carBooking.offcanvas')
