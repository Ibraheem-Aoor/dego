...
use Str;
class CarBookingController extends BaseCompanyController
{
    ...
    {
        $this->base_route_path .= 'car.booking.';
        $this->base_view_path .= 'car.booking.';
    ...
    public function all_booking_search(Request $request)
    {
        ...
        $Bookings = $this->model::query()->with(['user', 'depositable.gateway:id,name,image,driver', 'car:id,thumb,thumb_driver,name'])
            ->orderBy('id', 'DESC')
            // ->where('status', '!=', 0)
            ...
            ->when($status == 'pending', function ($query) {
                return $query->where('status', 1)->whereDoesntHave('bookingDates', function ($bookingDates) {
                    $bookingDates->whereDate('date', '<=', now());  // Ensure all dates are greater than today
                });
            })
            ...
            ->when($status == 'expired', function ($query) {
                return $query->where('status', 1)->whereDoesntHave('bookingDates', function ($bookingDates) {
                    $bookingDates->whereDate('date', '>=', now());  // Ensure all dates are less than today
                });
            })
            ...
            ->when(isset($filterStart) && !empty($filterStart), function ($query) use ($filterStart) {
                return $query->whereHas('bookingDates', function ($q) use ($filterStart) {
                    $q->whereDate('date', '>=', $filterStart);
                });
            })
        ...
        return DataTables::of($Bookings)
            ...
            ->addColumn('duration', function ($item) {
                return $item->getDuration() . ' Days';
            })
            ->addColumn('price', function ($item) {
                return currencyPosition($item->total_price) ?? 'N/A';

            })
            ->addColumn('car', function ($item) {
                $image = optional($item->car)->thumb;
                if (!$image) {
                    $firstLetter = substr($item->car->name, 0, 1);
                    return '<div class="avatar avatar-sm avatar-soft-primary avatar-circle d-flex justify-content-start gap-2 w-100">
                                ...
                                <p class="avatar-initials ms-3">' . $item->car->name . '</p>
                            </div>';

                } else {
                    $url = getFile(optional($item->car)->thumb_driver, optional($item->car)->thumb);
                    ...
                    return '<a class="d-flex align-items-center me-2" href="javascript:void(0)">
                                ...
                                <div class="flex-grow-1 ms-3">
                                  <h5 class="text-hover-primary mb-0">' . Str::limit($item->car->name, 30) . '</h5>
                                </div>
                              </a>';
            ...
            ->addColumn('status', function ($item) {
                ...
                } elseif ($item->status == 1 && strtotime($item->bookingDates()->max('date')) > strtotime(date('Y-m-d'))) {
            ...
            ->rawColumns(['action', 'checkbox', 'create-at', 'user', 'booking-id', 'car', 'status', 'duration', 'date', 'price'])
            ->make(true);
    }
    ...
    public function bookingEdit($id)
    {
        try {
            $data['booking'] = $this->model::with([
                ...
                'car' => function ($query) {
                    $query->select('id', 'name', 'pickup_location', 'drop_location');
                },
                ...
                ->where('id', $id)->firstOr(function () {
                    throw new \Exception('Car Booking Record not found.');
                });
