...
class PageController extends Controller
{
    ...
    public function index()
    {
        ...
        }])->whereNull('custom_link')->where('template_name', basicControl()->theme ?? 'relaxation')->get();
    ...
    public function update(Request $request, $id, $theme)
    {
        ...
        try {
            ...
            $selectedpage = Page::where('home_name',$request->home_name)->where('template_name', $configure->theme ?? 'relaxation')->first();
            ...
            if ($request->hasFile('breadcrumb_image')) {
                $image = $this->fileUpload($request->breadcrumb_image, config('filelocation.pagesImage.path'), null, null, 'webp', 80, $selectedpage->breadcrumb_image, $selectedpage->breadcrumb_image_driver);
                throw_if(empty($image['path']), 'Image could not be uploaded.');
                $breadCrumbImage = $image['path'];
                $breadCrumbImageDriver = $image['driver'] ?? 'local';
            }

            if ($request->status == 0 && $configure->home_style == $selectedpage->home_name){
                return back()->with('error','Only active home style cannot unpublished.');
            }

            $response = $selectedpage->update([
                "slug" => $request->slug,
                "template_name" => $theme,
                "breadcrumb_image" => $breadCrumbImage ?? $selectedpage->breadcrumb_image,
                "breadcrumb_image_driver" => $breadCrumbImageDriver ?? $selectedpage->breadcrumb_image_driver,
                "breadcrumb_status" => $request->breadcrumb_status,
                "status" => $request->status,
            ]);
            ...
            $selectedpage->details()->updateOrCreate([
                'language_id' => $request->language_id,
            ],
                [
                    "name" => $request->name,
    ...
    {
        $data['pageSEO'] = Page::findOrFail($id);
    ...
    public function pageSeoUpdate(Request $request, $id)
    {
        $request->validate([
            ...
            'og_description' => 'required|string|min:1|max:300',
