...
class ManageMenuController extends Controller
{
    public function manageMenu()
    {
        ...
        }])->whereIn('type',  [0,1])->whereIn('status', [0,1])->where('template_name', basicControl()->theme ?? 'relaxation')->get();
        $data['languages'] = Language::orderBy('default_status', 'desc')->get();
        $data['headerMenus'] = ManageMenu::where('menu_section', 'header')->where('theme', basicControl()->theme ?? 'relaxation')->firstOrFail();
        $data['footerMenus'] = ManageMenu::where('menu_section', 'footer')->where('theme', basicControl()->theme ?? 'relaxation')->firstOrFail();
    ...
    public function headerMenuItemStore(Request $request)
    {
        ...
        try {
            $menu = ManageMenu::where('menu_section', 'header')->where('theme',basicControl()->theme ?? 'relaxation')->firstOrFail();
    ...
    public function footerMenuItemStore(Request $request)
    {
        ...
        try {
            $menu = ManageMenu::where('menu_section', 'footer')->where('theme', basicControl()->theme ?? 'relaxation')->firstOrFail();
